...
def main():
    ...
    if page == "Overview":
        ...
        match_display = matches_df.apply(
            lambda row: f"{row['competition']} {row['season']} - {row['home_team']} vs {row['away_team']} ({row['date']})",
            axis=1
        )
        selected = st.selectbox("Select Match", match_display)
        selected_id = matches_df.iloc[match_display.tolist().index(selected)]["match_id"]

        # Load match events
        passes, xg_team1, xg_team2, times = load_match_events(selected_id)

        # Render simulation
        render_match_simulator(passes, xg_team1, xg_team2, times)
